...
import { Wrench, Loader2, CheckCircle2, AlertTriangle, Square } from "lucide-react";
...
export default function DuaContentFixerPanel() {
  ...
  const loadStats = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.functions.invoke("fix-dua-content", {
      body: { action: "stats" },
    });
    setLoading(false);
    if (error) {
  ...
  const start = useCallback(async () => {
    ...
    while (!stopRef.current && pending > 0) {
      ...
      if (error) {
        toast({ title: "Fix error", description: error.message, variant: "destructive" });
        break;
      }
    ...
    toast({
      ...
      description: `${totalSoFar} টি দোয়া ঠিক করা হয়েছে। ${failSoFar} টি ব্যর্থ।`,
    });
    loadStats();
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="rounded-lg border p-3 text-center">
            ...
            <p className="text-xs text-muted-foreground">অসম্পূর্ণ</p>
          </div>
        </div>
        ...
        <div className="flex gap-2">
          {!running ? (
            <Button onClick={start} disabled={loading || pending === 0} className="flex-1">
              ...
              {pending === 0 ? "সব দোয়া সম্পূর্ণ আছে" : `Fix Incomplete Dua Content (${pending} বাকি)`}
            </Button>
          ) : (
