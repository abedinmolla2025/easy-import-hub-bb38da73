...
export type ContentSeoGeneratorPanelProps = {
  /** admin_content.content_type this panel generates SEO for. */
  contentType: "dua" | "story";
  title?: string;
...
/**
 * Shared AI SEO generator panel (BN / EN / HI / UR) used by both the
 * Dua and Islamic Story admin modules. Backed by the `generate-dua-seo`
 * edge function, which accepts a `contentType` argument.
 */
export function ContentSeoGeneratorPanel({
  contentType,
  title = "Generate SEO Content (BN / EN / HI / UR)",
  description,
  totalLabel = "মোট আইটেম",
  batchSize = 10,
  functionName = "generate-dua-seo",
}: ContentSeoGeneratorPanelProps) {
  ...
  }, [toast, contentType, functionName]);
  ...
  const start = useCallback(async () => {
    ...
    while (!stopRef.current && pending > 0) {
      const { data, error } = await supabase.functions.invoke(functionName, {
        body: { action: "process", batchSize, contentType },
      });
      if (error) {
    ...
    toast({
      ...
      description: `${totalSoFar} টি আইটেমে SEO content যোগ হয়েছে। ${failSoFar} টি ব্যর্থ।`,
    });
    loadStats();
  }, [running, stats, toast, loadStats, batchSize, contentType, functionName]);
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {description ?? (
            <>
              প্রতিটি আইটেমের জন্য AI দিয়ে <code>explanation</code> (১০০–১৫০ শব্দ) ও <code>benefits</code> (৩–৫ পয়েন্ট)
              চারটি ভাষায় তৈরি করো — বাংলা, English, हिंदी, اردو। বিদ্যমান কন্টেন্ট overwrite হবে না; শুধু missing
              language fields পূরণ হবে। Batch size: {batchSize}।
            </>
          )}
        </CardDescription>
      </CardHeader>
        ...
        <div className="flex gap-2">
          {!running ? (
            <Button onClick={start} disabled={loading || pending === 0} className="flex-1">
              ...
              {pending === 0 ? "সব আইটেমের SEO content তৈরি হয়ে গেছে" : `Run All Missing (${pending} বাকি)`}
            </Button>
          ) : (
...
export default ContentSeoGeneratorPanel;
