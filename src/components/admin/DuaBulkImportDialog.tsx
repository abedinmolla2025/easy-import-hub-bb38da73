      ...
      if (toUpdate.length) {
        for (const it of toUpdate) {
          const { data, error } = await supabase
            .from("admin_content")
            .update({
              title_en: it.title_en as string,
              title_hi: it.title_hi as string,
              title_ur: it.title_ur as string,
              content_arabic: it.content_arabic as string,
              content: it.content_bn as string,
              content_en: it.content_en as string,
              content_hi: it.content_hi as string,
              content_ur: it.content_ur as string,
              content_pronunciation: it.pronunciation as string,
              content_pronunciation_en: it.pronunciation_en as string,
              content_pronunciation_hi: it.pronunciation_hi as string,
              content_pronunciation_ur: it.pronunciation_ur as string,
              category: it.category as string,
              source_type: it.source_type as string,
              reference: it.reference as string,
              hadith_reference: it.hadith_reference as string,
              explanation_bn: it.explanation_bn as string,
              explanation_en: it.explanation_en as string,
              explanation_hi: it.explanation_hi as string,
              explanation_ur: it.explanation_ur as string,
              benefits_bn: it.benefits_bn as string[],
              benefits_en: it.benefits_en as string[],
              benefits_hi: it.benefits_hi as string[],
              benefits_ur: it.benefits_ur as string[],
              when_to_recite_bn: it.when_to_recite_bn as string,
              when_to_recite_en: it.when_to_recite_en as string,
              when_to_recite_hi: it.when_to_recite_hi as string,
              when_to_recite_ur: it.when_to_recite_ur as string,
              subtitle: it.subtitle as string,
              authenticity: it.authenticity as string,
              difficulty: it.difficulty as string,
              time_required: it.time_required as string,
              hook: it.hook as string,
              share_text: it.share_text as string,
              virtue: it.virtue as string,
              virtue_reference: it.virtue_reference as string,
              viral_score: it.viral_score as number,
              audio_url: it.audio_url as string,
              emotion: it.emotion as string[],
              user_intents: it.user_intents as string[],
              recommendation_tags: it.recommendation_tags as string[],
              recommended_moments: it.recommended_moments as string[],
              semantic_entities: it.semantic_entities as string[],
              normalized_surah_names: it.normalized_surah_names as string[],
              related_duas: it.related_duas as string[],
              hook_variants: it.hook_variants as string[],
              social: it.social as any,
              og_image_data: it.og_image_data as any,
              seo: it.seo as any,
              quran_meta: it.quran_meta as any,
              category_hierarchy: it.category_hierarchy as any,
              faq: it.faq as any,
              search_aliases: it.search_aliases as any,
